...
namespace NT.Quotes.GQuoteMonitor
{
    public class QuoteMonitorUpdateManager : IQuoteMonitorUpdateManager
    {
        ...
        public void AddSymbol(string symbol)
        {
            lock (feedSyncLock)
            {
                ...
                {
                    _quoteUpdates.Add(symbol, new QuoteMonitorUpdateData());
                    _Instance.RequestQmonData(symbol);
        ...
        public void RemoveSymbol(string symbol)
        {
            ...
            lock (feedSyncLock)
            {
                ...
                {
                    _quoteUpdates[symbol].Count--;
                    //if (quoteUpdates[symbol].Count <= 0)
                    {
                        containsSymbol = true;
        ...
        public void Update(GQCore.TradeData trade)
        {
            lock (feedSyncLock)
            {
                if (_quoteUpdates.ContainsKey(trade.Symbol))
                {
                    if (trade.Last != double.NaN)
                    {
                    ...
                    {
                        if ((trade.PriceFlags & GQCore.PriceFlags.Set_Open) == GQCore.PriceFlags.Set_Open)
                        {
                            _quoteUpdates[trade.Symbol].Open = trade.Price;
                        }

                        if ((trade.PriceFlags & GQCore.PriceFlags.Set_High) == GQCore.PriceFlags.Set_High)
                        {
                            _quoteUpdates[trade.Symbol].High = trade.Price;
                        }

                        if ((trade.PriceFlags & GQCore.PriceFlags.Set_Low) == GQCore.PriceFlags.Set_Low)
                        {
                            _quoteUpdates[trade.Symbol].Low = trade.Price;
                        }
                        if ((trade.PriceFlags & GQCore.PriceFlags.Set_Close) == GQCore.PriceFlags.Set_Close)
                        {
                    ...
                    _quoteUpdates[trade.Symbol].PrecentChange = percentChange;
                    ...
                    {
                        _quoteUpdates[trade.Symbol].Exchange = GQCore.Active.NexxCoreExchanges.GetName(trade.ReportingExg);
                    }
        ...
        public void Update(GQCore.BBOQuoteData quote)
        {
            lock (feedSyncLock)
            {
        ...
        public bool IsOptionsymbol(GQCore.CompositeData data)
        {
        ...
        public void Update(GQCore.CompositeData data)
        {
            lock (feedSyncLock)
            {
                if (_quoteUpdates.ContainsKey(data.Symbol))
                {
                    ...
                    _quoteUpdates[data.Symbol].PExchgShort = GQCore.Active.NexxCoreExchanges.GetShortName(data.Exchange);
